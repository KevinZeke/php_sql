    ...
    public static function insert_score($hzdb, $date = null)
    {
        ...
        foreach (self::$db_map as $dd_name => $db) {
            ...
            $sql_tool->execute_dql_res(
                ...
                'SELECT * FROM `person_enforce_effects` ' . $date_sql
        ...
        if ($sql != '')
            return $hzdb->execute_dml(
                "INSERT INTO dadui_huizong_query_day (police_name,dd_name,year_month_show,effect_score) VALUES " . substr($sql, 1)
            );
        return 0;
    }
}

Effect::$db_map = get_all_dd_db_map();
<?php 
require_once __DIR__.'/DB_map.class.php';
class Gzpc_xmxx_jdjc_map extends DB_map {
  static $table_name = 'gzpc_xmxx_jdjc' ;
  static $id = 'gzpc_xmxx_jdjc.id' ;
  static $projectId = 'gzpc_xmxx_jdjc.projectId' ;
  static $projectType = 'gzpc_xmxx_jdjc.projectType' ;
  static $unitName = 'gzpc_xmxx_jdjc.unitName' ;
  static $unitType = 'gzpc_xmxx_jdjc.unitType' ;
  static $timeLimit = 'gzpc_xmxx_jdjc.timeLimit' ;
  static $director = 'gzpc_xmxx_jdjc.director' ;
  static $status = 'gzpc_xmxx_jdjc.status' ;
  static $createTime = 'gzpc_xmxx_jdjc.createTime' ;
  static $updateTime = 'gzpc_xmxx_jdjc.updateTime' ;
  static $overTime = 'gzpc_xmxx_jdjc.overTime' ;
  static $recordTime = 'gzpc_xmxx_jdjc.recordTime' ;
}
